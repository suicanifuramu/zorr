...
const {
    slugify, normalizeRarities, normalizeVariants, normalizePetals,
    normalizeMobs, normalizeTalents, normalizeBiomeMobs, computeSnakeIndices,
} = require('../normalizers');
const {
    isVariantMap, isRarityTupleArray, isPetalArray, isMobArray,
    isBiomeMobMap, detectSnakeProp, classify, SNAKE_PROP_NAMES,
...
test('normalizeVariants returns array keyed by numeric index', () => {
    ...
    assert.deepStrictEqual(out, [{ id: 3, name: 'Golden' }, { id: 7, name: 'Shiny' }]);
});

test('normalizePetals maps core fields', () => {
    const p = normalizePetals([{ id: 5, name: 'Rose', desc: 'heals', size: 2, damage: 3, health: 4, reload: 5, cost: 6 }]);
    assert.deepStrictEqual(p[0], { id: 5, name: 'Rose', slug: 'rose', desc: 'heals', size: 2, damage: 3, health: 4, reload: 5, cost: 6 });
});

test('normalizeMobs flags snakes via detectSnakeProp', () => {
    const mobs = [{ id: 1, name: 'Centipede', health: 10, damage: 2, armor: 0, size: 5, snakeCount: 3 }, { id: 2, name: 'Bee', health: 1, damage: 1, armor: 0, size: 1 }];
...
test('normalizeTalents requires slug, defaults parentId -1', () => {
    const t = normalizeTalents([{ id: 9, slug: 'sharper', cost: 3, value: 5, parentId: 2 }, { id: 10, name: 'no-slug' }]);
...
test('normalizeBiomeMobs expects mob-set objects, not arrays', () => {
    assert.deepStrictEqual(normalizeBiomeMobs({
...
test('shape classifier predicate helpers', () => {
    ...
    assert.strictEqual(isBiomeMobMap({ a: { x: 1, y: 2, z: 3 }, b: { x: 1, y: 2, z: 3 }, c: { x: 1, y: 2, z: 3 } }), true);
});
