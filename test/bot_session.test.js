...
const {
    LCG, MinHeap, decompressCoord, decodeItemValue,
    decodeStatusFlags, getPrintableAscii, decodeBuildCode,
    buildDistanceMap,
} = require('../bot_session');

test('LCG produces deterministic byte sequence', () => {
    const a = new LCG(42), b = new LCG(42);
    const seq = [];
    for (let i = 0; i < 10; i++) { const v = a.next(); seq.push(v); assert.strictEqual(v, b.next()); }
...
test('decodeBuildCode round-trips a payload', () => {
    ...
    let lcgState = 7 >>> 0;
    const next = () => { lcgState = (Math.imul(lcgState, 1664525) + 1013904223) >>> 0; return Math.floor(lcgState / 4294967296 * 255); };
...
test('decodeBuildCode rejects bad input', () => {
    ...
    const bad = Buffer.alloc(12); bad.writeUInt32BE(999, 0);
