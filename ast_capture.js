...
function findCandidates(source) {
    ...
    const collectFuncs = (node) => {
        ...
        if (Array.isArray(node)) {
            for (const x of node) collectFuncs(x);
            return;
        }
        ...
        if (node.type === "VariableDeclaration") {
            for (const d of node.declarations) {
                ...
                if (i.type === "ArrayExpression" && i.elements.length >= 5) {
                    ...
                    i.callee.object &&
                    i.callee.object.type === "Identifier" &&
                    i.callee.object.name === "JSON" &&
                    i.callee.property &&
                    i.callee.property.type === "Identifier" &&
                ...
                } else if (i.type === "Literal" && typeof i.value === "number" && i.value >= 420 && i.value <= 460) {
